...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.nbr_couleur.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.nbr_couleurs.update", [$nbr_couleur->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('nbr_coul') ? 'has-error' : '' }}">
                <label for="nbr_coul">{{ trans('global.nbr_couleur.fields.nbr_coul') }}*</label>
